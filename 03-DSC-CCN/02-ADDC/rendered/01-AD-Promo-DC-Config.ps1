...
Configuration FirstDomainController {
  ...
  Node $NodeName {
    ...
    # 0) BreakGlass LOCAL + canal de gestión (ANTES del bastionado)
    Script BreakGlassLocal {
      ...
      #Crea el usuario local de BreakGlass y lo añade a administradores locales. Si ya existe, se asegura que es admin local.
      SetScript  = {
    ...
    # Asegurar que el firewall permita WinRM (necesario para la promoción y gestión post-promoción).
    ...
    # El primer DC del bosque debe ser GC(Global Catalog) y la opción viene activada por defecto.
...
$ConfigData = @{
  AllNodes = @(
    @{
      ...
      # En producción, cifra credenciales con certificado
...
# Compilar y aplicar 
$Out = Join-Path $PSScriptRoot "ADDC-Config-Out"
if (Test-Path $Out) {
